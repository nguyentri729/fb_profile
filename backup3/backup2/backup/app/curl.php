<?php
	...
	$db = new MySQL('localhost', 'root', '', 'ios_app');
	date_default_timezone_set('Asia/Ho_Chi_Minh');
	?>
<?php
//$map_url = simplexml_load_file('https://ftios.vn/applications/main/store/install/3215');
/*if(preg_match_all('vip|vip.', $data)){
	echo 'là vip';
}*/

//find vip ipa
/*if(preg_match_all('#<string>http://vip.ftios.vn(.+?)</string>#is',$data, $_jickme)){
	if(isset())
	var_dump($_jickme);
}*/
set_time_limit(0);
$list_app = array();
for ($i=$_GET['i']; $i < $_GET['i_to']; $i++) { 
	@$data = file_get_contents('https://ftios.vn/applications/main/store/install/'.$i.'');
	
	if(preg_match_all('#<plist version="1.0">(.+?)</plist>#is',$data, $_jickme)){
		$xml_str  = $_jickme[0][0];
			if(strlen(strstr($xml_str, 'vip')) > 0) {
		...
		@$ipa_file = $xml->dict->array->dict->array->dict[0]->string[1];
		@$image = (string)$xml->dict->array->dict->array->dict[1]->string[1];
		@$name_fr = trim($xml->dict->array->dict->dict->string[3]);
		@$name = substr($name_fr, 0, -25);
		if($ipa_file && $image && $name){
