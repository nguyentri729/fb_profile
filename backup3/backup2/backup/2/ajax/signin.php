...
if($_POST['u'] && $_POST['p'] && $_POST['code'] && $orin == $BASEU || $orin == $BASEUs){
$code = str_replace(array('<',"'",'>','?','/',"\\",'--','eval','php'),array('','','','','','','','',''),strtoupper($_POST['code']));
...
$data = checkcode($code);
    if(isset($data['uid'])){
        addhethong($data['uid'],$u,$p,$data['idbot']);
        deletecode($code);
...
function checkuser($u){
      ...
      $result = mysqli_query($conn, "SELECT `uid`, `username`, `password`, `money` FROM `taikhoanhethongvip` WHERE `username`='$u' LIMIT 1");
