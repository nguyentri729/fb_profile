...
for($r = 0 ; $r < $cdt ;$r++){
    ...
    // $sleepafter = $datatoken[$r]['sleepafter'];
        //$runreac = reaction($token,$idstt,$id,$type);
            $runreac = reaction2($token,$idstt,$type);
            if($runreac == "true"){
                addhistorybot($id, $type,$idstt,$useradd);
                updatetotalauto($id,1);
            }
            deletenewfeedbotreaction($idstt);
}
die();



function reaction2($tokenn,$idstt,$type){
    $tokenn = trim($tokenn);
    $idstt = trim($idstt);
    $type = trim($type);
    $curl = curl_init();
    curl_setopt_array($curl, array(
      CURLOPT_URL => "https://graph.facebook.com/$idstt/reactions?access_token=$tokenn",
    ...
    if(isset($response['success'])){
        ...
        if($response['error']['code'] == '100' || $response['error']['code']=='200'){
            //deleteidstt($idstt);
            return "idsttdie";
        }
        if($response['error']['code'] == '3'){
           updateago($tokenn,'999999999999999');
        }
    }
}
...
function comment($toke,$id,$text,$urlimagecmt,$packsticker){
    ...
    $text = urlencode($text);
    $a = file_get_contents("https://graph.facebook.com/$id/comments?method=post&message=".$text.$contentstickrequest.$contentimagerequest."&access_token=".$toke);
    if(strpos($a,'"id"')){
        return true;
    }else{
        return false;
...
function getdatatoken($limit = 50){
      global $conn;
      $nows = strtotime(date('Y-m-d H:i:s'));
      //$nowHi = date("H:i:s");
      $result = mysqli_query($conn, "SELECT * FROM `newfeedbotreaction` WHERE '$nows'-`created_time`>`sleep` LIMIT $limit");
      $kq = array();
      while ($row = $result->fetch_assoc()){
        ...
        //   $sleepafter = $row['sleepafter'];
          $kq[] = array('token'=>$token,'id'=>$id,'type'=>$type,'idstt'=>$idstt,'useradd'=>$useradd);
...
function addnewfeedbotreaction($token,$idstt,$uid,$type,$created_time,$sleepafter){
      global $conn;
      mysqli_query($conn, "INSERT INTO `newfeedbotreaction`(`idstt`, `token`, `type`, `created_time`, `sleep`,`uid`) VALUES ('$idstt','$token','$type','$created_time','$sleepafter','$uid')");    
}
function deletenewfeedbotreaction($idstt){
      global $conn;
      mysqli_query($conn, "DELETE FROM `newfeedbotreaction` WHERE `idstt`='$idstt'");    
}
